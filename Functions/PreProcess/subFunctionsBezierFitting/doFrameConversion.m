function [lengh,ey,eYaw,kappa,kappaPol] = doFrameConversion(pCar,yawCar,tol,bezierCurves,distIteration)
    epsilon = 0.00001;
    pTarget = [pCar(1),pCar(2),yawCar];
    numberOfCurves = numel(bezierCurves(1,1,:));
    distCurve = zeros(numberOfCurves,1);
    tCurve = zeros(numberOfCurves,1);
    for i = 1:1:numberOfCurves
        curve = bezierCurves(:,:,i);
        %plotBezierCurves(curve, i -1);
        [tCurve(i),distCurve(i)] = findClosestPoint(curve,tol,pTarget(1:2));
    end
    [~,closedCurve] = min(distCurve);
    lengh = 0;
    step = 0.001;
    for i = 1:1:closedCurve - 1
        lengh = lengh + calcLengthOfSegment(bezierCurves(:,:,i),step,1);
    end
    lengthTillCurve = lengh;
    ...
    eYaw = atan2(dir(2),dir(1)) - pTarget(3);
    while eYaw > pi
        eYaw = eYaw - 2 * pi;
    end
    while eYaw < -pi
